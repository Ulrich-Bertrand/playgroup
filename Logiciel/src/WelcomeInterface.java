...
public class WelcomeInterface extends JFrame implements ActionListener{
	...
	/**
	 * cette classe est celle qui permet d'afficher la premiere activité de notre projet. C'est elle qui contient le main
	 * Elle contient deux panneaux: 
	 * 1-celui du Construsteur de la classe Menus venant du meme repertoire
	 * 2- un definit en tant que variable (connex)  qui va servire de conteneur pour les options du menu apres leurs effectivité
	 * 
	 */
	public WelcomeInterface() {
		
		/*la class Defaults ne contient que les parametres de l'ecran et deux ou trois fonctions
		*qui definissent juste la taille des composants Ã  la fenetre principale
		*/
		...
		/**
		 * Admin : class créé une boite de dialogue pour demande de connexion en tant qu'administrateur
		 * de ce block il est interdit de penser proceder à l'inscription d'un nouveau administrateur dans la banque.
		 */
		admin = new Admin() ; 
		
		/**
		 * de meme que pour un administrateur à la differnce de prevoir une option inscrivez-vous pour un 
		...
		// ici on definit juste les boutons des menus se retrouvant au cote gauche de l'écran
		...
		this.setTitle("Account Management"); // donne le titre ï¿½ la fenetre
